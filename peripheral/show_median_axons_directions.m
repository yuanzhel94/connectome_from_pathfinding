function f = show_median_axons_directions(c,median_axons,show,n_dim)
...
%             plot(axoni(:,1),axoni(:,2),'color',color_code,'LineWidth',weight_log/max_weight_log*(max_linewidth-1)+1);
            axoni_length = size(axoni,1);
            for j = 1:(axoni_length-1)
                vecj = axoni(j+1,:) - axoni(j,:);
                lengthj = sqrt(sum(vecj .^2,2));
                color_code = abs(vecj) ./ lengthj;
                color_code = [color_code,0];
                plot(axoni(j:j+1,1),axoni(j:j+1,2),'color',color_code,'LineWidth',weight_log/max_weight_log*(max_linewidth-1)+1);
            end
        elseif n_dim == 3
%             plot3(axoni(:,1),axoni(:,2),axoni(:,3),'color',color_code,'LineWidth',weight_log/max_weight_log*(max_linewidth-1)+1);
            ...
            for j = 1:(axoni_length-1)
                ...
                lengthj = sqrt(sum(vecj .^2,2));
                color_code = abs(vecj) ./ lengthj;
                plot3(axoni(j:j+1,1),axoni(j:j+1,2),axoni(j:j+1,3),'color',color_code,'LineWidth',weight_log/max_weight_log*(max_linewidth-1)+1);
            end
        else
