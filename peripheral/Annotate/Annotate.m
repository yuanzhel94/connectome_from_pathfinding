...
% Date: 03-31-2020
    ...
    properties (SetAccess = private)
        Primitive % primitive annotation object handle
    end

    properties (Dependent = true)
        Position % axes data coordinate position
    end
    ...
    methods
        ...
        function obj = Annotate(hAxes, annotation_type, xdata, ydata, varargin)
            ...
            if ishg2()
                ...
                if isa(hContainer,'matlab.graphics.layout.TiledChartLayout')
                    hContainer = get(hContainer,'parent');
                end
        ...
        function xy = get.Position(obj)
            ...
            if ishg2()
                % w.r.t. axes parent
                if isuifigure(hFig)
                    axpos = get(hAxes,'innerposition');
                    if ~strcmpi(get(hAxes,'units'),'pixels')
                        axpos = hgconvertunits(hFig, axpos, get(hAxes,'units'), 'pixels', obj.ContainerHandle);
                    end
                else
        ...
        function set.UIContextMenu(obj, hMenus)
            ...
            uicm_obj = uicontextmenu(obj.FigureHandle);
