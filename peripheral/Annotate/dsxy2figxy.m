...
%   2023-08-12: works for uiaxes embedded in uifigure
%   2023-03-31: works for axes embedded in tiled chart layout
...
if ishg2()
    ...
    if isa(hContainer,'matlab.graphics.layout.TiledChartLayout')
        % w.r.t. tiled chart layout parent
        hContainer = get(hContainer,'parent');
    end
    if isuifigure(hFig)
        axpos = get(hAx,'innerposition');
        if ~strcmpi(get(hAx,'units'),'pixels')
            axpos = hgconvertunits(hFig, axpos, get(hAx,'units'), 'pixels', hContainer);
        end
    else
